...
var TrackSender = class {
  ...
  async _sendAsFile(validFiles, workInfo, session) {
    ...
    const rjCode = `RJ${String(workInfo.id).padStart(8, "0")}`;
    const downloadWorker = /* @__PURE__ */ __name(({ index, file }) => this._downloadWithRetry(file.url, file.title).then((buffer) => ({ status: "fulfilled", value: { buffer, file, index } })).catch((error) => ({ status: "rejected", reason: error, index, title: file.title })), "downloadWorker");
  ...
  async handleSingleZip(validFiles, workInfo, session) {
    ...
    try {
      const rjCode = `RJ${String(workInfo.id).padStart(8, "0")}`;
      const downloadWorker = /* @__PURE__ */ __name(({ index, file }) => this._downloadWithRetry(file.url, file.title).then((buffer) => ({
