...
var Config = import_koishi.Schema.intersect([
  import_koishi.Schema.object({
    apiBaseUrl: import_koishi.Schema.string().default("https://api.asmr-200.com/api").description("用于获取音声数据的 API 地址。\n\n`asmr-300.com 随缘墙`\n\n`asmr-200.com 随缘墙`\n\n`asmr-100.com 国内墙`\n\n`asmr.one 国内墙`\n\n`替换格式:https://api.{域名}/api`"),
  ...
  import_koishi.Schema.object({
    ...
    downloadTimeout: import_koishi.Schema.number().default(300).description("单个音轨下载的超时时间（秒）。")
  }).description("下载与发送设置"),
  import_koishi.Schema.object({
  ...
  import_koishi.Schema.union([
    import_koishi.Schema.object({
      ...
      password: import_koishi.Schema.string().role("secret").default("").description("压缩包密码。")
    }),
    import_koishi.Schema.object({})
  ])
]);
        ...
        for (const result of results) {
          if (result.status === "fulfilled") {
            ...
            if (downloadOutcome.status === "fulfilled") {
              ...
              await session.send(`下载音轨 ${downloadOutcome.index} 「${import_koishi.h.escape(downloadOutcome.title)}」失败，已跳过。`);
            }
          }
        }
        if (downloadedFiles.length > 0) {
          const zipFilename = getZipFilename(workInfo.title);
          await session.send(`下载完成 ${downloadedFiles.length} 个文件，正在创建压缩包 「${import_koishi.h.escape(zipFilename)}」...`);
        ...
        if (downloadOutcome.status === "fulfilled") {
          if (downloadOutcome.value.buffer.byteLength > 100) {
            ...
            try {
              ...
              await session.send(`正在发送文件: 「${import_koishi.h.escape(track.title)}」`);
              ...
              await session.send(`发送音轨 ${downloadOutcome.index} 「${import_koishi.h.escape(track.title)}」失败。`);
            } finally {
...
可附加 card | file | zip 选项，例如 "1 2 3 card"`);
    ...
    if (config.useForward && session.platform === "onebot") {
      ...
      data.works.forEach((work, index) => {
        ...
        const entryText = [
          ...
          `   声优: 🎤 ${import_koishi.h.escape(work.vas.map((v) => v.name).join(", ") || "未知")}`
        ].join("\n");
  ...
  ctx.command("热门音声 [page:number]", "获取当前热门音声列表").action(async ({ session }, page = 1) => {
    ...
    const onNextPage = /* @__PURE__ */ __name((nextSession, nextPage) => handleListInteraction(nextSession, nextPage, fetcher, "热门音声", onNextPage), "onNextPage");
